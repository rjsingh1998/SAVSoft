...
public class VilleHome {
	...
	public void delete(Ville persistentInstance) {
		...
		try {
			...
			//System.out.println("suppression ville impossible!!!!!!!!!!!!");
			log.error("delete failed", re);
			throw re;
		}
	}

	public Ville merge(Ville devilledInstance) {
		log.debug("merging Ville instance");
		try {
			Ville result = (Ville) sessionFactory.getCurrentSession()
					.merge(devilledInstance);
	...
	@SuppressWarnings("unchecked")
	public List<Ville> findAll() {//select * from ville;
	...
	@SuppressWarnings("unchecked")
	public List<Ville> findByNom(String n) {//select * from ville where cond;
		Criteria crit = sessionFactory.getCurrentSession().createCriteria(
				Ville.class).add(Restrictions.eq("designationVille", n));
		return crit.list();
	}
	
	@SuppressWarnings("unchecked")
	public List<Ville> findAllWithFilter(String valeurRecherche) {
		Criteria crit = sessionFactory.getCurrentSession().createCriteria(Ville.class)
				.add(Restrictions.like("designationVille", valeurRecherche, MatchMode.ANYWHERE));
		return crit.list() ;
	}
